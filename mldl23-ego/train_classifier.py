...
def main():
    ...
    models = {}
    s_name  = '/content/info.txt'
    my_dict = {} 
    my_dict['model_list'] =   model_list
    my_dict['args.models'] = args.models
    my_dict['args.batch_size'] = args.batch_size
    my_dict['args.models_dir'] = args.models_dir
    my_dict['num_classes'] = num_classes          
    ...
    for m in modalities:
        ...
        my_dict[' models[{}]'.format(m)] =  models[m]
    ...
    # the models are wrapped into the ActionRecognition task which manages
    ...
    if args.action == "train":
        ...
        #     my_dict3['A.shape'] = A.shape
        #     my_dict3['B.shape'] = B.shape
...
def train(action_classifier, train_loader, val_loader, device, num_classes):
    ...
    for i in range(iteration, training_iterations):
        ...
        try:
            ...
            source_data, source_label = next(data_loader_source)
        end_t = datetime.now()
        my_dict['source_label'] =source_label
        with open(s_name, 'a') as f:
...
def validate(model, val_loader, device, it, num_classes):
    ...
    with torch.no_grad():
        ...
        class_accuracies = [(x / (y+0.000000000001)) * 100 for x, y in zip(model.accuracy.correct, model.accuracy.total)]
